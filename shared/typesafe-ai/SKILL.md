...
The TypeSafe SDKs and HTTP API authenticate with `TYPESAFE_API_KEY`. Before you
run code or tests that call the API, check that the variable is set in the
session, for example with `printenv TYPESAFE_API_KEY | wc -c` so the value is
not printed. When it is missing, do not produce a call that will fail with
`401`. Tell the user to create a key at https://console.typesafe.ai/keys and to
set the variable in their shell, in the `env` block of Claude Code's
`settings.json`, or in `shell_environment_policy.set` of Codex's `config.toml`.
Designing questions and writing integration code does not need the key; only
executing the calls does. Never write the key into source files or commit it.
